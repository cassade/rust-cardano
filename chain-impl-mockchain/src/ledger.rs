...
pub enum Error {
    ...
    Block0InitialMessageNoConsensusLeaderCert,
...
struct EmptyLedgerBuilder {
    ...
    consensus_leader_certs: Vec<LeaderId>,
}
...
impl EmptyLedgerBuilder {
    pub fn try_with_config_param(mut self, param: &ConfigParam) -> Result<Self, Error> {
        match param {
            ...
            ConfigParam::ConsensusLeaderCert(param) => {
                self.consensus_leader_certs.push(param.clone());
                None
            }
    ...
    pub fn build(self, block0_initial_hash: HeaderHash) -> Result<Ledger, Error> {
        ...
        match consensus_leader_certs.len() {
            0 => return Err(Error::Block0InitialMessageNoConsensusLeaderCert),
            _ => settings.bft_leaders = Arc::new(consensus_leader_certs),
        }
...
pub mod test {
    ...
    pub fn utxo() -> () {
        ...
        ie.push(ConfigParam::ConsensusLeaderCert(LeaderId::from(
            leader_pub_key,
        )));
